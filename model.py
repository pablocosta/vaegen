...
class SenteceVAE(nn.Module):
    def __init__(self, vocabSize, embeddingsSize=50, recurrenceType="rnn", hiddenSize=80, wordDropout=0.5, embeddingDropout=0.1, latentSize=25, maxSequenceLenght=10,
        ...
        self.encoder = rnn(self.embeddingsSize, self.hiddenSize, num_layers=self.numLayers,
                           bidirectional=self.bidirectional, batch_first=True)
        self.decoder = rnn(self.embeddingsSize, self.hiddenSize, num_layers=self.numLayers,
                           bidirectional=self.bidirectional, batch_first=True)

        self.hiddenFactor = (2 if self.bidirectional else 1) * self.numLayers

        self.hidden2mean = nn.Linear(
            self.hiddenSize * self.hiddenFactor, latentSize)
        self.hidden2logv = nn.Linear(
            self.hiddenSize * self.hiddenFactor, latentSize)
        self.latent2hidden = nn.Linear(
            self.latentSize, self.hiddenSize * self.hiddenFactor)
        self.outputs2vocab = nn.Linear(
            self.hiddenSize * (2 if self.bidirectional else 1), self.vocabSize)
    ...
    def forward(self, inputSequence, lenght):
        ...
        packedInput = nn.utils.rnn.pack_padded_sequence(
            inputEmbeddings, sortedLenght.data.tolist(), batch_first=True)
        # rnn pass by
        ...
        if self.wordDropout > 0:
            ...
            prob[(inputSequence.data - SOSToken) *
                 (inputSequence.data - PADToken) == 0] = 1
        ...
        packedInput = rnn_utils.pack_padded_sequence(
            inputEmbeddings, sortedLenght.data.tolist(), batch_first=True)

        # decoder forward pass
        outputs, _ = self.decoder(packedInput, hidden)
        # process outputs
        paddedOutputs = rnn_utils.pad_packed_sequence(
            outputs, batch_first=True)[0]
        ...
        logp = nn.functional.log_softmax(self.outputs2vocab(
            paddedOutputs.view(-1, paddedOutputs.size(2))), dim=-1)
    ...
    def inference(self, n=4, z=None):
        ...
        sequenceIdx = torch.arange(
            0, batchSize, out=self.tensor()).long()  # all idx of batch
        ...
        generations = self.tensor(
            batchSize, self.maxSequenceLenght).fill_(PADToken).long()

        t = 0
        while t < self.maxSequenceLenght and len(runSequences) > 0:

            if t == 0:
                inSequence = (torch.Tensor(batchSize).fill_(
                    SOSToken).long()).cuda()
            ...
            generations = self._save_sample(
                generations, inSequence, runSequences, t)
            ...
            if len(runSequences) > 0:
                ...
                runSequences = torch.arange(
                    0, len(runSequences), out=self.tensor()).long()
